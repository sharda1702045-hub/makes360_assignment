...
<a href="{{ $href }}" 
   class="flex items-center px-3 py-2 rounded-xl transition-colors duration-200 group {{ $active ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"
>
    <div class="shrink-0">
        {{-- Use simple SVG paths for icons based on type --}}
        @if($icon === 'dashboard')
    ...
    <span class="ml-3 text-sm font-medium overflow-hidden whitespace-nowrap" x-show="sidebarOpen">{{ $slot }}</span>
</a>
